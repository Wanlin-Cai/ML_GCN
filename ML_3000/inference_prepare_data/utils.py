...
def split_dataset(args, dataset):
    ...
    if args['split'] == 'scaffold_decompose':
        ...
        train_set, val_set, test_set = RandomSplitter.train_val_test_split(
            dataset, frac_train=train_ratio, frac_val=val_ratio, frac_test=test_ratio,random_state=1024)
    else:
...
def load_model(exp_configure):
    if exp_configure['model'] == 'GCN':
        ...
        model = GCNPredictor(
            ...
            activation=[(lambda x: F.leaky_relu(x, negative_slope=0.2))] * exp_configure['num_gnn_layers'],
